...
std::map<std::string, levmuRequest::COMMAND> levmuRequest::cmd_map;
bool _ignored = levmuRequest::init_cmd_map();

levmuRequest::levmuRequest(levmuServer *server, 
                           const muduo::net::TcpConnectionPtr& conn)
    :server_(server),
     db_index(0),
     arg_count(-1),
     name(""),
     response_(new levmuResponse(conn)) {
}

void levmuRequest::_run() {
...
void levmuRequest::run() {
    _run();
}

bool levmuRequest::init_cmd_map() {
    cmd_map["set"]              = &levmuRequest::levmu_set;
    cmd_map["get"]              = &levmuRequest::levmu_get;
    cmd_map["incr"]             = &levmuRequest::levmu_incr;
    cmd_map["incrby"]           = &levmuRequest::levmu_incrby;

    return true;
}

void levmuRequest::levmu_set() {
...
void levmuRequest::levmu_incrby(){
